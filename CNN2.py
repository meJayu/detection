# -*- coding: utf-8 -*-
"""
Created on Thu May 25 19:22:52 2017

simple CNN model for training the network

Weights initialization: Xavier initialization

Updates from CNN.py
1) SGD without momentum using variable learning rate(step learning rate)

    - learning rate is decreased by 1/2 every 5 epoches
...
num_filters2 = 24         # There are 36 of these filters.
...
layer_fc3 = new_fc_layer(input=layer_fc2_drop,
                         num_inputs=fc_size2,
...
l1_regularizer = tf.contrib.layers.l1_regularizer(scale=WEIGHT_DECAY_FACTOR, scope=None)
...
def optimize(epoch):
    ...
    while e in range(total_epoch,
        ...
        for batch in range(1,11):
            ...
            while i < num_test:
                ...
                feed_dict_train = {x: images,
                              y_true: labels,keep_prob:0.5,lr:l_rate}  
        ...
        if e % 1 == 0:
            ...
            acc = print_test_accuracy(batch=11)
            
            #save the result
            accV.append(acc)
            
            # Message for printing.
            msg = "Optimization Epoch: {0:>6}, Test Accuracy: {1:>6.1%}"
...
def print_test_accuracy(batch=12):
...
def plot_images(images, cls_true, cls_pred=None,img_shape=img_shape):
    ...
    M = sio.loadmat('/home/jpatel/Python/Dataset/meanVal.mat')
    meanVal = M['meanVal']
    
    S = sio.loadmat('/home/jpatel/Python/Dataset/stdVal.mat')
    stdVal = S['stdVal']
...
def download_variables(n = 8):
    ...
    # A feed-dict is not necessary because nothing is calculated.
    for i in range(n):
        name = 'weights_conv%d'%(n)
        w = session.run(name)
        Weights['name']=w
    
    #Save the weights    
    sio.savemat("Weights_CNN2.mat",Weights)
...
def get_data(n):
    ...
    M = sio.loadmat('/home/jpatel/Python/DatasetMAT_192/meanVal.mat')
    meanVal = M['meanVal']
    
    #S = sio.loadmat('/home/jpatel/Python/DatasetMAT_192/stdVal.mat')
    #stdVal = S['stdVal']
    
    #n = batch number 
    
    #load the .mat file
    filename='/home/jpatel/Python/DatasetMAT_192/batch%d.mat'%(n)
    ...
    return (np.array(a,dtype=np.float32)/255-np.array(meanVal)),np.array(b,dtype=np.int)
